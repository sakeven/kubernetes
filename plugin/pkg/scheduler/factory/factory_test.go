...
func TestDefaultErrorFunc(t *testing.T) {
	...
	mux.Handle(testapi.Default.ResourcePath(string(v1.ResourcePods), "bar", "foo"), &handler)
	...
	for {
		...
		handler.ValidateRequest(t, testapi.Default.ResourcePath(string(v1.ResourcePods), "bar", "foo"), "GET", nil)
...
func TestBind(t *testing.T) {
	...
	for _, item := range table {
		...
		handler.ValidateRequest(t,
			testapi.Default.SubResourcePath(string(v1.ResourcePods), metav1.NamespaceDefault, "foo", "binding"),
			"POST", &expectedBody)
	}
}
